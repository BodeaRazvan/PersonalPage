...
init : () -> ( Model, Cmd Msg )
init _ =
    ( update GetRepos initModel
    )

getRepository: Cmd Msg
getRepository = Http.get{
    url ="https://api.github.com/users/BodeaRazvan/repos",
    expect = Http.expectJson GotRepos (De.list Repo.decodeRepo)
    }

addRepository: Model -> List Repo.Repo -> Model
addRepository  model repository = {
    personalDetails = model.personalDetails,
    events = model.events,
    selectedEventCategories = model.selectedEventCategories,
    repos = repository
    }
...
update msg model =
    case msg of
        ...
        GotRepos res ->
            case res of
                Ok repos -> ( addRepository model repos, Cmd.none )
