...
calculateAbsolute: Int -> Int -> Int
calculateAbsolute x y =
    if(x > y) then
        x - y
    else
        y - x
...
monthsBetween dA dB =
   ...
   case (date1.month,date2.month) of
        (Just m1, Just m2) -> Just (abs(12 * (date1.year - date2.year) + (monthToInt m1 - monthToInt m2)))
        (Nothing, Nothing) -> Just (12 * calculateAbsolute date1.year date2.year)
        _ -> Nothing
...
compare (Date d1) (Date d2) =
       case (d1.month,d2.month) of
        ...
        (Nothing,Nothing) -> chainCompare EQ (Basics.compare d1.year d2.year)
        (Just m1,Nothing) -> chainCompare LT (Basics.compare d1.year d2.year)
        (Nothing,Just m2) -> chainCompare GT (Basics.compare d1.year d2.year)
...
view (Date d) =
    div[] <|
    [p[][text (String.fromInt d.year ++ " " ++ case d.month of
...
monthsBetweenMonths m1 m2 =
   let
    month1 = monthToInt m1
    month2 = monthToInt m2
   in
