...
compare (Interval intA) (Interval intB) =
     case (intA.end, intB.end) of
        (Nothing,Nothing) -> chainCompare EQ (Date.compare intA.start intB.start)
        (Just end1, Just end2) -> chainCompare (Date.compare end1 end2) (Date.compare intA.start intB.start)
        (Just end1, Nothing) -> chainCompare LT (Date.compare intA.start intB.start)
        (Nothing, Just end2) -> chainCompare GT (Date.compare intA.start intB.start)

view : Interval -> Html msg
view interval =
    let
        startDate = case interval of Interval i -> i.start
        endDate = case  interval of Interval i -> i.end
    in
     div [class "interval"] <|
      [p[class "interval-start"][Date.view startDate],
       p[class "interval-end"](case endDate of
                               Nothing -> [text "Present"]
                               Just x -> [Date.view x]),
       case length interval of
           Nothing -> p[][]
           Just (x,y) -> p[class "interval-length"][text ("Years: " ++ (String.fromInt x) ++ " Months:" ++ (String.fromInt y))]
      ]
